...
const DishList: React.FC<DishListProps> = ({
    ...
    const handleAutoSelect = (id: string) => {
        const obj: WaiterDish | undefined = dishes.find(value => value.id === id);

        if (obj) {
            onDishToggle(id);
            const objs: WaiterDish[] = dishes.filter(value => value.tableNumber === obj.tableNumber);
            objs.forEach(value => onDishToggle(value.id))
            console.log(objs);
        }

    }

    const handler = (mess: string) => {
        (async () => {
            await onRequestRemake(mess);
        })()
    }
                            ...
                            {items.map((dish) => (
                                <li key={dish.id}>
                                    <div
                                        ...
                                        onClick={() => {
                                            if (activeTab !== 'đã phục vụ') {
                                                console.log(dish.id)
                                                handleAutoSelect(dish.id)
                                            }
