...
import {Bell, Users, Clock, User , CreditCard, UserCheck, ChefHat, Filter} from 'lucide-react';
...
const ModeratorScreen: React.FC = () => {
    ...
    const statusCounts = {
        notification: Object.values(data).filter(table => (table.counter || 0) > 0).length,
        paid: Object.values(data).filter(table => 
            table.totalItems > 0 && table.paidCount / table.totalItems === 1 && (table.counter || 0) === 0
        ).length,
        serving: Object.values(data).filter(table => 
            table.totalItems > 0 && table.paidCount / table.totalItems < 1 && (table.counter || 0) === 0
        ).length,
        normal: Object.values(data).filter(table => 
            table.totalItems === 0 && (table.counter || 0) === 0
        ).length
    };
                    ...
                    {filteredData
                        ...
                        .map(([tableId, tableData]) => {
                            ...
                            const status = (() => {
                                ...
                                if (tableData.totalItems > 0) {
                                    if (tableData.paidCount / tableData.totalItems === 1) return 2;
                                    return -1;
                                }
                                return 0;
                            })()
                                ...
                                >
                                    ...
                                    <div className="absolute bottom-2 left-2 flex justify-evenly flex-wrap gap-y-4 items-center gap-3">
                                         ...
                                         <div className="flex items-center gap-1 text-black text-xs font-semibold bg-white/80 rounded px-1">
                                            <ChefHat size={12} />
                                            <span>{tableData.serveredCount || 0}/{tableData.totalItems || 0}</span>
                                        </div>
                                        {/* Delivered count */}
                                        <div className="flex items-center gap-1 text-black text-xs font-semibold bg-white/80 rounded px-1">
                                            <UserCheck size={12} />
                                            <span>{tableData.deliveredCount || 0}/{tableData.totalItems || 0}</span>
                                        </div>
                                        ...
                                        <div className="flex items-center gap-1 text-black text-xs font-semibold bg-white/80 rounded px-1">
                                            <CreditCard size={12} />
                                            <span>{tableData.paidCount || 0}/{tableData.totalItems || 0}</span>
                                        </div>
                                    </div>
