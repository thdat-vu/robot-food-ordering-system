...
export const ModeratorFeedbackFromTable: React.FC<Prop> = ({
  ...
  const determineInitialTab = (
    feedbackData: FeedbackgGetTableId[]
  ): TabType => {
  ...
  useEffect(() => {
    if (open && idTable && activeTab !== null) {
      if (activeTab === "feedback" && data.length === 0) {
        ...
        (!orderData[idTable] || orderData[idTable].length === 0)
      ) {
        fetchOrdersForTable(idTable);
      }
    }
  ...
  const filteredData = data.filter((item) => {
    ...
    const matchesSearch =
      ...
      (item.resolutionNote &&
        item.resolutionNote.toLowerCase().includes(searchQuery.toLowerCase()));
          ...
          <div className="relative z-10">
            ...
            {activeTab === "feedback" && (
              <div className="grid grid-cols-3 gap-4">
                <div className="bg-white/20 backdrop-blur-lg rounded-2xl px-4 py-3 text-center border border-white/30">
              ...
              <div className="grid grid-cols-4 gap-4">
                <div className="bg-white/20 backdrop-blur-lg rounded-2xl px-4 py-3 text-center border border-white/30">
                  <div className="flex items-center justify-center space-x-2 mb-1">
                    ...
                    <span className="text-2xl font-bold">
                      {orderData[idTable]?.filter((o) => o.status === "Pending")
                        .length || 0}
                    </span>
                  </div>
            ...
            <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-4">
              ...
              <div className="flex items-center space-x-4">
                ...
                >
                  <RefreshCw
                    className={`w-5 h-5 ${
                      isLoading || loadingOrders[idTable] ? "animate-spin" : ""
                    }`}
                  />
                </button>
              </div>
                        ...
                        <div className="flex items-start space-x-6">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between mb-4">
                              <div className="flex items-center space-x-4">
                                <span className="bg-white px-4 py-2 rounded-full text-sm font-bold text-gray-600 shadow-sm">
                                  #{index + 1}
                                </span>
                                <div
              ...
              {activeTab === "feedback" ? (
                <>
                  <p className="text-gray-600 font-medium">
                    ...
                    / <span className="font-bold">{data.length}</span> phản hồi
                  </p>
                  {searchQuery && (
