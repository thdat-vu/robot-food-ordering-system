...
const OrderDetailDialog: React.FC<OrderDetailDialogProps> = ({
    ...
    for (const item of items) {
      ...
      if (map.has(key)) {
        ...
        const toppingMap = new Map<
          string,
          { name: string; price: number; count: number; totalPrice: number }
        >();
        [...existing.toppings, ...item.toppings].forEach((topping) => {
          if (toppingMap.has(topping.id)) {
            const existTopping = toppingMap.get(topping.id)!;
            existTopping.count += 1;
            existTopping.totalPrice += topping.price;
          } else {
            toppingMap.set(topping.id, {
              name: topping.name,
              price: topping.price,
              count: 1,
        ...
        // replace toppings with merged list
        existing.toppings = Array.from(toppingMap.values()) as any;
      } else {
        // khởi tạo item mới
        const toppingMap = new Map<
          string,
        ...
        map.set(key, {
          ...item,
          toppings: Array.from(toppingMap.values()) as any,
        });
      }
    }
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-2 sm:p-4">
      <div className="bg-white rounded-2xl shadow-2xl relative w-full max-w-6xl max-h-[95vh] sm:max-h-[90vh] overflow-hidden">
        {/* Dialog Header - Responsive */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between p-4 sm:p-6 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-indigo-50">
          <div className="flex-1 mb-3 sm:mb-0">
            <h2 className="text-xl sm:text-2xl font-bold text-gray-900 flex items-center">
              <AlertCircle className="w-5 h-5 sm:w-6 sm:h-6 mr-2 sm:mr-3 text-blue-600 flex-shrink-0" />
              <span className="truncate">Xem Lại Thay Đổi Trạng Thái - {table.name}</span>
            </h2>
            <p className="text-xs sm:text-sm text-gray-600 mt-1">
              ...
              <span
                className={`font-medium ${
                  newStatus === "0" ? "text-green-600" : "text-red-600"
                }`}
              >
          ...
          <button
            className="w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center rounded-full hover:bg-gray-100 transition-colors duration-200 text-gray-500 hover:text-gray-700 self-end sm:self-center"
            onClick={onClose}
          >
        ...
        >
          {loading ? (
            <div className="text-center py-8 sm:py-12">
              ...
              <p className="text-gray-600 text-sm sm:text-base">Đang tải đơn hàng để xem lại...</p>
            </div>
          ) : orders && orders.length > 0 ? (
            <div className="space-y-4 sm:space-y-6">
              {/* Summary Card - Responsive Grid */}
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-4 sm:p-6 border border-blue-200">
                {/* Main Statistics - 2x2 Grid on Mobile, 4 columns on Tablet+ */}
                <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                  {/* Đơn Hàng Hoạt Động */}
                  <div className="text-center bg-white/50 rounded-lg p-3 sm:p-4">
                    <div className="flex items-center justify-center mb-2">
                      <ShoppingCart className="w-5 h-5 sm:w-6 sm:h-6 text-blue-600 mr-2" />
                  ...
                  <div className="text-center bg-white/50 rounded-lg p-3 sm:p-4">
                    <div className="flex items-center justify-center mb-2">
                      <Users className="w-5 h-5 sm:w-6 sm:h-6 text-indigo-600 mr-2" />
                      <div className="text-xl sm:text-2xl font-bold text-indigo-600">
                        {new Set(orders.map(o => o.tableId)).size}
                      </div>
                    </div>
                    <div className="text-xs sm:text-sm text-gray-600 font-medium">
                      Bàn Phục Vụ
                ...
                {/* Payment Status - Full width on Mobile, 2 columns on Tablet+ */}
                <div className="border-t border-blue-200 pt-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    {/* Đơn chưa thanh toán */}
                    <div className="flex items-center justify-between bg-red-50 px-4 py-3 rounded-lg border border-red-200">
                      <div className="flex items-center space-x-3">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-center space-y-1 sm:space-y-0 sm:space-x-4 mt-1">
                              <span className="flex items-center space-x-1 text-xs sm:text-sm">
                                ...
                                <span className="truncate">{getOrderStatusLabel(order.status)}</span>
                              </span>
                              <span className="flex items-center space-x-1 text-xs sm:text-sm">
                                {getPaymentStatusIcon(order.paymentStatus)}
                                <span className="truncate">{getPaymentStatusLabel(order.paymentStatus)}</span>
                              </span>
                            </div>
                          </div>
                        </div>
                          ...
                          {mergeOrderItems(order.items).map((item, idx) => (
                            ...
                            >
                              ...
                              {item.toppings.length > 0 && (
                                <div className="mt-2 ml-0 sm:ml-14 text-xs sm:text-sm text-gray-600 space-y-1">
        ...
        <div className="flex flex-col sm:flex-row justify-end space-y-2 sm:space-y-0 sm:space-x-3 p-4 sm:p-6 border-t border-gray-200 bg-gray-50">
          <button
            ...
            className="w-full sm:w-auto px-4 sm:px-6 py-2 sm:py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors flex items-center justify-center font-medium text-sm sm:text-base"
          >
          ...
          <button
            ...
            className="w-full sm:w-auto px-4 sm:px-6 py-2 sm:py-2 bg-gradient-to-r from-green-500 to-green-600 text-white rounded-lg hover:from-green-600 hover:to-green-700 transition-all shadow-md hover:shadow-lg flex items-center justify-center font-medium text-sm sm:text-base"
          >
