...
export const QuickServeModal: React.FC<QuickServeModalProps> = ({
    ...
    Object.values(tables).forEach((table) => {
      if (table.listComplain && table.listComplain.length > 0) {
        table.listComplain.forEach((c) => {
          ...
          if (
            desc.includes("cho thêm") ||
            desc.includes("cho them") ||
  ...
  const filteredComplains = useMemo(() => {
    ...
    return allQuickComplains.filter(
      (c) =>
        c.description.toLowerCase().includes(q) ||
        c.tableName.toLowerCase().includes(q) ||
        (c.quickServeItems &&
          c.quickServeItems.some((item) => item.toLowerCase().includes(q)))
    );
  ...
  const handleSelectAll = () => {
    if (
      selectedIds.size === filteredComplains.length &&
    ...
    try {
      ...
      for (const tId of tableIds) {
        ...
        await runCheck(
          tId,
          fbIds,
          "Nhân viên đã tiếp nhận yêu cầu phục vụ nhanh",
          false
        );
      }
          ...
          <div className="flex items-center gap-6 relative z-10">
            ...
            <div>
              <h2 className="text-4xl font-extrabold tracking-tight mb-2 flex items-center gap-3">
                Quick Serve{" "}
                <span className="text-sm font-medium bg-white/20 px-3 py-1 rounded-full backdrop-blur-md border border-white/20">
                  Moderator Tool
                </span>
              </h2>
              <p className="text-indigo-100 text-lg font-medium opacity-90">
                Xử lý nhanh các yêu cầu "Cho thêm" từ khách hàng
              </p>
            </div>
          </div>
          ...
          {filteredComplains.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center py-20">
              ...
              <h3 className="text-3xl font-bold text-gray-800 mb-4">
                Không có yêu cầu "Cho thêm"
              </h3>
              <p className="text-gray-500 text-xl max-w-md mx-auto leading-relaxed">
                Hiện tại không có phản hồi nào chứa yêu cầu phục vụ nhanh hoặc
                gia vị.
              </p>
            </div>
          ) : viewMode === "list" ? (
                ...
                >
                  <div className="flex items-center gap-6">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xl font-bold text-gray-800 line-clamp-2 leading-relaxed">
                        {c.description}
                      </p>
                      ...
                      {c.quickServeItems && c.quickServeItems.length > 0 && (
                        <div className="mt-4 flex flex-wrap gap-2">
                          {c.quickServeItems.map((item, idx) => (
                            <span
                              key={idx}
                              className="bg-emerald-50 text-emerald-700 border border-emerald-100 px-3 py-1 rounded-xl text-xs font-bold uppercase tracking-wider"
                            >
                              + {item}
                            </span>
                          ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredComplains.map((c) => (
                ...
                >
                  ...
                  <h4 className="text-2xl font-black text-gray-800 mb-6 leading-tight flex-1">
                    {c.description}
                  </h4>
                  ...
                  {c.quickServeItems && c.quickServeItems.length > 0 && (
                    <div className="mt-auto pt-6 border-t border-gray-100/50 flex flex-wrap gap-2">
                      {c.quickServeItems.map((item, idx) => (
                        <span
                          key={idx}
                          className="bg-emerald-50 text-emerald-700 border border-emerald-100 px-3 py-1.5 rounded-xl text-xs font-bold uppercase tracking-wider"
                        >
                          {item}
                        </span>
                      ))}
        ...
        <div className="p-8 bg-white border-t border-gray-100 flex items-center justify-between shadow-[0_-10px_20px_rgba(0,0,0,0.02)]">
          <div className="flex items-center gap-4">
            <div className="bg-indigo-50 text-indigo-700 px-6 py-4 rounded-2xl border border-indigo-100">
              <span className="text-3xl font-black mr-3">
                {selectedIds.size}
              </span>
              <span className="font-bold text-indigo-900/60 uppercase tracking-widest text-xs">
                Mục đã chọn
              </span>
            </div>
