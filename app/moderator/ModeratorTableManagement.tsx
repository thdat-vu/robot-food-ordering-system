...
export default function ModeratorTableManagement() {
  ...
  const debouncedFetchTables = useCallback(debounce(fetchTables, 300), [
  ...
  const deboundfetchOrdersForTable = useCallback(
    debounce(fetchOrdersForTable, 300),
    []
  );
          ...
          <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100">
            ...
            {loading ? (
              ...
              <>
                ...
                <div
                  className={`${
                    viewMode === "grid" ? "hidden md:block" : "hidden md:block"
                  } overflow-x-auto`}
                >
                          ...
                          <td className="py-4 px-6">
                            <div className="flex items-center">
                              ...
                              <div>
                                ...
                                {orderData[row.id] &&
                                  orderData[row.id].length > 0 && (
                                    <div className="text-sm text-gray-500 flex items-center mt-1">
                                      <Users className="w-3 h-3 mr-1" />
                                      {orderData[row.id].reduce(
                                        (acc, order) =>
        ...
        <OrderDetailDialog
          ...
          orders={
            selectedTableForOrders
              ? orderData[selectedTableForOrders.id] || []
              : []
          }
          loading={
            selectedTableForOrders
              ? loadingOrders[selectedTableForOrders.id] || false
              : false
          }
          onConfirmStatusChange={(reason?: string) =>
            confirmStatusChange(reason)
          }
