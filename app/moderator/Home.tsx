...
export const Home: React.FC<{ idTable: string }> = ({idTable}) => {
    ...
    const fetchTableDetail = async () => {
        try {
            ...
            addToast(error?.response?.data?.errorMessage || "Không thể tải thông tin bàn", "error");
        } finally {
            setLoading(false);
        }
    };
    ...
    const confirmStatusChange = async (reason?: string) => {
        ...
        try {
            ...
            const errorMessage = err?.response?.data?.errorMessage || "Không thể cập nhật trạng thái bàn";
    ...
    const executeToggleBlock = async (reason?: string) => {
        ...
        try {
            ...
            console.log(`Mock ${willBeBlocked ? 'blocking' : 'unblocking'} table ${idTable}`, {
                reason: reason || 'No reason provided'
            });

            await new Promise(resolve => setTimeout(resolve, 1000));

            setTableData(prev => prev ? {...prev, isBlocked: willBeBlocked} : null);

            addToast(
                `${willBeBlocked ? '🔒 Đã khóa' : '🔓 Đã mở khóa'} bàn thành công (DEMO - chưa lưu vào backend)`,
                "success"
            );
            ...
            const errorMessage = error?.response?.data?.errorMessage
                ...
                || error?.message
                || `Không thể ${willBeBlocked ? 'khóa' : 'mở khóa'} bàn. Vui lòng thử lại.`;
    ...
    const getStatusInfo = (status: number) => {
        ...
        switch (statusValue) {
            case 0:
                return {label: 'Trống', color: 'bg-green-100 text-green-800 border-green-200', icon: CheckCircle};
            case 1:
                return {label: 'Có Khách', color: 'bg-red-100 text-red-800 border-red-200', icon: AlertCircle};
            case 2:
                return {label: 'Đã Đặt', color: 'bg-yellow-100 text-yellow-800 border-yellow-200', icon: AlertCircle};
            default:
                return {label: 'Không xác định', color: 'bg-gray-100 text-gray-800 border-gray-200', icon: AlertCircle};
        }
    };
        ...
        return (
            <div className="flex items-center justify-center h-full min-h-[500px]">
                <div className="text-center">
                    ...
                    <h3 className="text-xl font-semibold text-gray-800 mb-2">Không tìm thấy bàn</h3>
    ...
    const qrCodeUrl = getQRCodeUrl(400);
    const computedStatus = pendingStatus !== null ? pendingStatus : getNextStatus(tableData.status);
            ...
            <div className="h-full">
                <div className="mb-6">
                    <div className="flex items-center justify-between">
                        <div>
                            <h2 className="text-3xl font-bold text-gray-800 mb-2">{tableData.name}</h2>
                            <div className="flex items-center space-x-3">
                                ...
                                {tableData.isBlocked && (
                                    <span
                                        className="inline-flex items-center space-x-2 px-4 py-2 rounded-full text-sm font-semibold bg-red-100 text-red-800 border-2 border-red-200 animate-pulse">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="bg-white rounded-2xl shadow-lg border-2 border-gray-200 p-8">
                        ...
                        <div
                            className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl p-6 mb-6 flex items-center justify-center">
                            <div className="bg-white rounded-xl p-6 shadow-lg border-2 border-gray-200">
                                {qrImageError ? (
                                    <div className="w-80 h-80 flex flex-col items-center justify-center text-gray-400">
                                        <AlertCircle className="w-16 h-16 mb-4"/>
                                        <p className="text-sm text-center font-medium">Không thể tải mã QR</p>
                                        <button
                                            onClick={() => {
                                                setQrImageError(false);
                                                fetchTableDetail();
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-white rounded-2xl shadow-lg border-2 border-gray-200 p-8">
                            <div className="flex items-center space-x-3 mb-6">
                                ...
                                <h3 className="text-xl font-bold text-gray-800">Thay Đổi Trạng Thái Bàn</h3>
                            </div>
                            ...
                            <div className={`rounded-xl p-6 mb-6 border-2 ${
                                ...
                                <div className="flex items-start space-x-4 mb-4">
                                    <StatusIcon className={`w-8 h-8 flex-shrink-0 mt-1 ${
                                        isAvailableStatus(tableData.status) ? 'text-green-600' : 'text-red-600'
                                    }`}/>
                                    <div>
                                        <h4 className={`text-lg font-bold mb-2 ${
                                            isAvailableStatus(tableData.status) ? 'text-green-800' : 'text-red-800'
                                        }`}>
                                            Trạng thái hiện tại: {statusInfo.label}
                                        </h4>
                                        <p className={`text-sm ${
                                            isAvailableStatus(tableData.status) ? 'text-green-700' : 'text-red-700'
                                        }`}>
                                            {isAvailableStatus(tableData.status)
                            ...
                            <div className="bg-gray-50 rounded-xl p-6 mb-6">
                                ...
                                <ul className="space-y-3">
                                    <li className="flex items-start space-x-3">
                                        ...
                                        <span className="text-sm text-gray-700"><strong>Trống:</strong> Bàn sẵn sàng đón khách mới</span>
                                    </li>
                                    <li className="flex items-start space-x-3">
                                        <div
                                        ...
                                        <span className="text-sm text-gray-700"><strong>Có Khách:</strong> Bàn đang được sử dụng</span>
                                    </li>
                                    <li className="flex items-start space-x-3">
                                        <div
                                            className="w-5 h-5 rounded-full bg-yellow-100 flex items-center justify-center flex-shrink-0 mt-0.5">
                                            <span className="text-yellow-600 text-xs font-bold">2</span>
                                        </div>
                                        <span className="text-sm text-gray-700"><strong>Đã Đặt:</strong> Bàn đã được đặt trước</span>
                                    </li>
                                </ul>
                            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
                    ...
                    <div
                        ...
                        <div className="flex items-center space-x-3 mb-2">
                            ...
                            <span className="text-sm font-semibold text-purple-800">Trạng thái</span>
                        </div>
                    ...
                    <div
                        className="bg-gradient-to-br from-emerald-50 to-emerald-100 rounded-xl p-4 border-2 border-emerald-200">
                        <div className="flex items-center space-x-3 mb-2">
                            {tableData.isBlocked ? (
                            ...
                            <span className="text-sm font-semibold text-emerald-800">Bảo mật</span>
                        </div>
                        <p className="text-xs text-emerald-700">
                            {tableData.isBlocked ? 'Đã khóa truy cập (Demo)' : 'Cho phép đặt món'}
                        </p>
                    </div>
                </div>
            </div>
